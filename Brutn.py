...
with open('APIKeys.txt','r') as f:
    for line in f:
        ETHAPI, BSCAPI, POLAPI = line.split(':')
...
def subforce():
    ...
    while True:    
        ...
        #POL
        try:
        	pol =requests.get(f'https://api.polygonscan.com/api?module=account&action=txlist&address={addr}&startblock=0&endblock=99999999&page=1&offset=10&sort=asc&apikey={POLAPI}', timeout=5000.0)
        except requests.exceptions.RequestException as err:
            print(colored("Request Error" , color="red"))
            logging.warning(err)
        ...
        polJson =pol.json()
        dumppolJson = json.dumps(polJson)

        loadpolJson = json.loads(dumppolJson)
        polTransaction = loadpolJson["status"]
        print(colored("pol" , color="white"))
        print(polTransaction)
        if int(polTransaction) >0:
            print(colored(f"Has transaction history {me} {addr}", color="green"))
            with open("valid.txt", "a") as polWallets:
                    polWallets.write("\nWallet: " + me + " pol CHAIN " + addr)
        else:
            print(colored(f"{polTransaction} {me} {addr}", color="yellow"))
            with open("empty.txt", "a") as polWallets:
                    polWallets.write("\nWallet: " + me + " pol CHAIN " + addr)
...
def mainforce():
    ...
    print("Your pol API key is: ")
    print(colored(f"{POLAPI}", color="yellow"))   
